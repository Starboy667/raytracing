...
static int IMSTB_TEXTEDIT_GETNEXTCHARINDEX_IMPL(ImGuiInputTextState* obj,
    ...
    unsigned int c;
    return idx + ImTextCharFromUtf8(&c, obj->TextA.Data + idx,
                                    obj->TextA.Data + obj->TextLen);
}
...
static int IMSTB_TEXTEDIT_GETPREVCHARINDEX_IMPL(ImGuiInputTextState* obj,
    ...
    if (idx <= 0) return -1;
    const char* p =
        ImTextFindPreviousUtf8Codepoint(obj->TextA.Data, obj->TextA.Data + idx);
...
static int is_word_boundary_from_right(ImGuiInputTextState* obj, int idx) {
    ...
    const char* prev_p =
        ImTextFindPreviousUtf8Codepoint(obj->TextA.Data, curr_p);
    unsigned int curr_c;
...
static void InputTextReconcileUndoStateAfterUserCallback(
    ...
    for (first_diff = 0; first_diff < shorter_length; first_diff++)
        if (old_buf[first_diff] != new_buf_a[first_diff]) break;
    if (first_diff == old_length && first_diff == new_length_a) return;

    int old_last_diff = old_length - 1;
    int new_last_diff = new_length_a - 1;
    for (; old_last_diff >= first_diff && new_last_diff >= first_diff;
         old_last_diff--, new_last_diff--)
        if (old_buf[old_last_diff] != new_buf_a[new_last_diff]) break;
    ...
    if (g.ActiveId == id && !g.ActiveIdIsJustActivated && !clear_active_id) {
        ...
        if (IsKeyPressed(ImGuiKey_LeftArrow)) {
            ...
            if (g.PlatformIO.Platform_SetClipboardTextFn != NULL) {
                ...
                state->TextA.Data[ie] =
                    0;  // A bit of a hack since SetClipboardText only takes
        ...
        if (apply_edit_back_to_user_buffer) {
            ...
            if ((flags & (ImGuiInputTextFlags_CallbackCompletion |
                ...
                if (event_flag) {
                    ...
                    memcpy(state->CallbackTextBackup.Data, state->TextA.Data,
                           state->TextLen + 1);
                    ...
                    if (buf_dirty) {
                        ...
                        InputTextReconcileUndoStateAfterUserCallback(
                            ...
                            callback_data
                                .BufTextLen);  // FIXME: Move the rest of this
                                               // block inside function and
                                               // rename to
...
// This handle some subtleties with capturing info from the label, but for 99%
// uses it could essentially be rewritten as:
//    if (ImGui::BeginChild("...", ImVec2(ImGui::CalcItemWidth(),
//    ImGui::GetTextLineHeight() * 7.5f), ImGuiChildFlags_FrameStyle))
//        { .... }
//    ImGui::EndChild();
//    ImGui::SameLine();
...
bool ImGui::TabItemEx(ImGuiTabBar* tab_bar, const char* label, bool* p_open,
    ...
    if (tab_contents_visible &&
        ...
        if (x_offset < 2.0f * g.CurrentDpiScale) x_offset = 0.0f;
